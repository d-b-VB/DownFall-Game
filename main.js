...
const zones = [
  ...
  { id: 'tundra', name: 'Tundra', palette: ['#8faecc', '#c8d8e6', '#f5fbff'], waypoint: '🧊', enemy: '😠', unlock: null, ring: ring(3, 4), arc: [11.5, 12.5] },
  ...
  { id: 'outer_a', name: 'Outer Reach A', palette: ['#4f6f8f', '#6487ad', '#85abd0'], waypoint: '🧭', enemy: '😠', unlock: null, ring: ring(4, 5), arc: [16, 20] },
  { id: 'outer_b', name: 'Outer Reach B', palette: ['#4f7f65', '#68a081', '#86c2a2'], waypoint: '🧭', enemy: '😠', unlock: null, ring: ring(4, 5), arc: [22, 14] },
];
...
const inArc = (hour, [start, end]) => { const h = wrap24(hour), s = wrap24(start), e = wrap24(end); return s <= e ? h >= s && h < e : h >= s || h < e; };
const toClockHour = (theta) => wrap24((theta * 6 / Math.PI) + 6); // rotated so volcano/frost on left
...
function resetWorld(){ const s=worldXY(25,20); state.player={x:s.x,y:s.y,vx:0,vy:0,hp:20,weapon:'club',unlocked:new Set(['club']),swing:0,lastX:s.x,lastY:s.y}; state.projectiles=[]; state.enemies=[]; state.terrain=[]; state.waves={}; for(const z of zones){ state.waves[z.id]={spawned:false,cleared:false}; const mid=wrap24((z.arc[0]+z.arc[1])*0.5); const r=(z.ring.min+z.ring.max)*0.5; const th=(mid-6)*Math.PI/6; const bx=MAP_CENTER.x+Math.cos(th)*r, by=MAP_CENTER.y+Math.sin(th)*r; state.terrain.push({x:bx-70,y:by-40,hp:3,glyph:'🌲'}); state.terrain.push({x:bx+45,y:by+20,hp:3,glyph:'🪨'});} }
