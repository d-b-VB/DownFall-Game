...
const BUILD_VERSION = 'v0.2.4 build 2026-06-01 00:00 UTC';
...
function metaFor(id){ if(!state.meta[id]) state.meta[id]={speed:0,range:0,damage:0,knockback:0,accel:0,poison:0,poisonResist:0}; return state.meta[id]; }
...
function addStatus(o,type,power=1){o.status=o.status||{}; if(type==='poison')o.status.poison=(o.status.poison||0)+power; if(type==='fire')o.status.fire=(o.status.fire||0)+10*power; if(type==='ice')o.status.ice=Math.max(o.status.ice||0,4*power);}
function tickStatus(o,dt,isPlayer=false){if(!o.status)return; const max=maxHpOf(o); if(o.status.poison){const res=isPlayer?Math.min(0.85,(metaFor('player').poisonResist||0)*0.1):0,pot=1+(metaFor('poison').poison||0)*0.1,rate=0.01*Math.sqrt(o.status.poison)*pot*(1-res); o.hp=Math.max(0,o.hp-(max-o.hp)*rate*dt);} if(o.status.fire>0){const dps=Math.max(0,o.status.fire); o.hp=Math.max(0,o.hp-dps*dt); o.status.fire=Math.max(0,o.status.fire-dt);} if(o.status.ice>0){o.status.ice=Math.max(0,o.status.ice-dt);} }
...
const coldRiverPath=[{h:10.5,r:3.8},{h:11.5,r:3.35},{h:0.4,r:3.05},{h:1.7,r:3.15},{h:2.8,r:3.45},{h:3.35,r:4.45}];
const warmCreekPath=[{h:7.5,r:3.75},{h:8.4,r:2.9},{h:9.7,r:2.0},{h:11.1,r:1.45},{h:0.8,r:1.75},{h:2.4,r:2.65},{h:3.2,r:3.55},{h:3.55,r:4.45}];
...
function update(dt){const p=state.player; let dx=(state.keys.has('d')?1:0)-(state.keys.has('a')?1:0),dy=(state.keys.has('s')?1:0)-(state.keys.has('w')?1:0); const m=Math.hypot(dx,dy)||1;dx/=m;dy/=m;
  const zone=getZone(p.x,p.y), flow=riverPushAt(p.x,p.y); if(flow.damage)p.hp=Math.max(0,p.hp-flow.damage*dt); tickStatus(p,dt,true); if(zone?.id!==state.currentZone){state.currentZone=zone?.id||null; if(zone)dbg(`[ZONE] entered ${zone.name} wave=${state.waves[zone.id]?.wave||1} spawned=${!!state.waves[zone.id]?.spawned}`);} const horseEq=state.mount==='horse'&&p.unlocked.has('horse'); const horse=metaFor('horse'); const speed=horseEq?2.25*(1+horse.speed*0.1):1,accel=horseEq?4.8*(1+horse.accel*0.1):7.5;
  ...
  for(const e of state.enemies){tickStatus(e,dt,false); const bird=e.zone==='fletcher', em=(e.power||1); e.vx=(e.vx||0)*(bird?0.985:0.88); e.vy=(e.vy||0)*(bird?0.985:0.88); const ax=p.x-e.x,ay=p.y-e.y,d=Math.hypot(ax,ay)||1,ep=projectWorld(e.x,e.y),off=ep.x<-40||ep.x>innerWidth+40||ep.y<-40||ep.y>innerHeight+40,chaseMul=off?3.2:1; if(e.zone==='creek'){e.hop=(e.hop||0)-dt; if(e.hop<=0){let tx=ax/d,ty=ay/d; const frogs=state.enemies.filter(o=>o!==e&&o.zone==='creek'); let nearest=null,nd=Infinity; for(const o of frogs){const od=Math.hypot(o.x-e.x,o.y-e.y); if(od<nd){nd=od;nearest=o;}} if(nearest&&Math.random()<0.45){tx=(e.x-nearest.x)/(nd||1);ty=(e.y-nearest.y)/(nd||1);} e.vx+=tx*135*chaseMul; e.vy+=ty*135*chaseMul; e.hop=0.45+Math.random()*0.75;}} else { const accel=(bird?130:12)*Math.min(2.6,em); e.vx += ax/d*accel*dt*chaseMul; e.vy += ay/d*accel*dt*chaseMul; if(!bird){e.x += ax/d*12*dt*chaseMul*Math.min(2,em); e.y += ay/d*12*dt*chaseMul*Math.min(2,em);}} const ef=waterPushAt(e.x,e.y); e.x += (e.vx+ef.x)*dt; e.y += (e.vy+ef.y)*dt; if(ef.damage)e.hp-=ef.damage*dt*0.35; const er=Math.hypot(e.x-MAP_CENTER.x,e.y-MAP_CENTER.y); if(e.minR&&er<e.minR){const sc=e.minR/Math.max(1,er);e.x=MAP_CENTER.x+(e.x-MAP_CENTER.x)*sc;e.y=MAP_CENTER.y+(e.y-MAP_CENTER.y)*sc;e.vx*=0.3;e.vy*=0.3;} if(e.zone==='creek'&&getZone(e.x,e.y)?.id!=='creek'){const cz=zones.find(z=>z.id==='creek'),pt=randomPointInZone(cz);e.x=pt.x;e.y=pt.y;e.vx=0;e.vy=0;}} resolveEnemySpacing(); hitChecks(); if(p.hp<=0){resurrect();return;}
