...
const BUILD_VERSION = 'v0.28.0 build 2026-06-24 00:18 UTC';
...
function cheatMenuHtml(){
  ...
  const upgradeHtml=zones.map(zone=>{const opts=cheatUpgradeOptionsForZone(zone);if(!opts.length)return '';return `<details><summary>${zone.name}</summary>${opts.map((o,i)=>{const symbol=o.unlock||o.elementUnlock?'🔓':UPGRADE_SYMBOLS[o.stat]||'◆';return `<button type="button" data-cheat-upgrade="${zone.id}:${i}" data-tip="${o.label}">${symbol} ${o.label}</button>`;}).join(' ')}</details>`;}).join('');
  ...
  return `<div class="shop-modal cheat-modal"><strong>Cheat Mode — paused</strong><button type="button" data-close-cheat="1">Close</button><div class="shop-section"><strong>Health</strong><br>Current <button data-cheat-health="hp" data-cheat-delta="-1">−</button><strong>${state.player.hp.toFixed(1)}</strong><button data-cheat-health="hp" data-cheat-delta="1">+</button> Max <button data-cheat-health="max" data-cheat-delta="-1">−</button><strong>${state.player.maxHp}</strong><button data-cheat-health="max" data-cheat-delta="1">+</button></div><div class="shop-section"><button type="button" data-cheat-clear="1">☠ Auto kill enemies and clear zone</button> <button type="button" data-cheat-stock="all">📦 Stock all consumables</button></div><div class="shop-section"><strong>Consumables</strong><br>${consumables}</div><div class="shop-section"><strong>All upgrades</strong>${upgradeHtml}</div></div>`;
}
