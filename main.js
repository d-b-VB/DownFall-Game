...
const CONSUMABLE_QTY={arrows:6,bolts:3,jars:2,pellets:8,cannonballs:2,fire:2,ice:2,poison:2};
const CONSUMABLE_HEALS={fruit:1,meat:2,cheese:3,stew:4};
const CONSUMABLE_HOME={arrows:'fletcher',bolts:'sawmill',jars:'creek',pellets:'smith',cannonballs:'foundry',fruit:'orchard',meat:'axe',cheese:'pasture',stew:'inn',cloak:'tanner',caltrops:'farrier',decoy:'armorer',fire:'volcano',ice:'frost',poison:'swamp',antitoxin:'swamp'};
const ESCALATING_CONSUMABLES=new Set(['fruit','meat','cheese','stew','cloak','caltrops','decoy']);
const BULK_DISCOUNT_CONSUMABLES=new Set(['jars','pellets']);
...
function consumableAmount(kind){return Math.max(1,state.consumableAmounts[kind]||1);}
function adjustConsumable(kind,delta){state.consumableAmounts[kind]=Math.max(1,Math.min(99,consumableAmount(kind)+delta));}
function consumablePriceAt(kind,offset=0){const base=CONSUMABLE_BASE[kind],bought=(state.shopPurchases[purchaseKey(kind)]||0)+offset;if(base===undefined)return undefined;return base+(ESCALATING_CONSUMABLES.has(kind)?bought:0);}
function consumableBulkPrice(kind,amount=consumableAmount(kind)){let total=0;for(let i=0;i<amount;i++)total+=consumablePriceAt(kind,i)||0;if(BULK_DISCOUNT_CONSUMABLES.has(kind)&&amount>1)total=Math.max(1,Math.ceil(total*(1-.08*Math.min(3,amount-1))));return total;}
function canBuyItem(kind,z){
  const home=CONSUMABLE_HOME[kind];
  if(home&&z!==home)return `[SHOP] ${kind} is sold in ${home}`;
  if(kind==='cloak'&&(state.player.cloaks||0)>=1)return '[SHOP] only one elemental cloak can be carried';
  if(kind==='arrows'&&!state.player.unlocked.has('bow'))return '[SHOP] unlock bow before buying arrows';
  if(kind==='bolts'&&!state.player.unlocked.has('ballista'))return '[SHOP] unlock ballista before buying bolts';
  if(kind==='jars'&&!state.player.unlocked.has('sling'))return '[SHOP] unlock sling before buying jars';
  if(kind==='cannonballs'&&!state.player.unlocked.has('cannon'))return '[SHOP] unlock cannon before buying cannonballs';
  if(['fire','ice','poison'].includes(kind)&&!state.player.unlockedElements.has(kind))return `[SHOP] unlock ${kind} before buying charges`;
  return null;
}
function grantConsumable(kind,amount=1){
  if(CONSUMABLE_HEALS[kind]){const before=state.player.hp;state.player.hp=Math.min(state.player.maxHp,state.player.hp+CONSUMABLE_HEALS[kind]*amount);return dbg(`[HEAL] ${kind} +${(state.player.hp-before).toFixed(1)} HP`);}
  ...
  if(kind==='caltrops'){state.player.caltrops=(state.player.caltrops||0)+amount;return dbg(`[SHOP] caltrops +${amount}`);}
  if(kind==='decoy'){state.player.decoys=(state.player.decoys||0)+amount;return dbg(`[SHOP] armor-stand decoy +${amount}`);}
  if(['fire','ice','poison'].includes(kind)){state.elements[kind]+=CONSUMABLE_QTY[kind]*amount;state.activeElement=kind;return dbg(`[SHOP] ${kind} charges +${CONSUMABLE_QTY[kind]*amount}`);}
  state.ammo[kind]+=CONSUMABLE_QTY[kind]*amount;dbg(`[SHOP] ${kind} +${CONSUMABLE_QTY[kind]*amount}`);
}
function buyItem(kind){
  const amount=consumableAmount(kind),here=getZone(state.player.x,state.player.y),z=(state.pendingReward&&here?.id===state.pendingReward.zone)?state.pendingReward.zone:state.currentZone,blocked=canBuyItem(kind,z);if(blocked)return dbg(blocked);
  const cost=consumableBulkPrice(kind,amount);if(cost===undefined)return;if(state.diamonds<cost)return dbg(`[SHOP] need ${cost}♦ for ${amount}× ${kind}`);state.diamonds-=cost;const key=purchaseKey(kind);state.shopPurchases[key]=(state.shopPurchases[key]||0)+amount;grantConsumable(kind,amount);dbg(`[SHOP] bought ${amount}× ${kind} for ${cost}♦`);
}
...
function quantityControl(kind,value){return `<button data-consumable-adjust="-1" data-kind="${kind}">−</button><strong>${value}×</strong><button data-consumable-adjust="1" data-kind="${kind}">+</button>`;}
function shopConsumables(zoneId){
  ...
  let html=items.map(([kind,label])=>{const full=kind==='cloak'&&(state.player.cloaks||0)>=1,amount=consumableAmount(kind),cost=consumableBulkPrice(kind,amount);return `<span class="qty-buy">${quantityControl(kind,amount)} <button type="button" data-buy="${kind}" ${full?'disabled':''}>${full?'🧥 cloak already equipped':`${label} · ${cost}♦`}</button></span>`;}).join(' ');
...
function resetWorld(){const s={x:MAP_CENTER.x,y:MAP_CENTER.y};state.player={x:s.x,y:s.y,vx:0,vy:0,hp:8+(metaFor('player').maxHp||0),maxHp:8+(metaFor('player').maxHp||0),status:{},weapon:'club',lastWeapon:'club',unlocked:new Set(['club']),unlockedElements:new Set(),shield:false,enchants:{},swing:0,swingSerial:0,attackCooldown:0,attackCooldownTotal:0,cannonCooldown:0,lastX:s.x,lastY:s.y}; state.mount='foot'; state.ammo={arrows:0,bolts:0,jars:0,pellets:0,cannonballs:0}; state.slingMode='rock'; state.arrowMode='regular'; state.elements={fire:0,ice:0,poison:0}; state.activeElement=null; state.projectiles=[];state.pickups=[];state.enemies=[];state.terrain=[];state.hazards=[];state.deployables=[];state.feedback=[];state.damagePreview=null;state.hudFlash={hp:0,diamonds:0,lastHp:state.player.hp,lastDiamonds:state.diamonds};state.fungusRespawns={};state.nextEnemyId=1;state.waves={};state.pendingReward=null;state.deferredRewards=[];state.shopOpen=false;state.cheatOpen=false;state.offerNpc=null;state.currentZone=null;state.finance={savings:0,debt:0,trust:0,trustAvailable:0,amounts:{savings:5,loan:10,trust:5}};state.casinoWagers={coin:1,dice:1,card:1};state.shopPurchases={}; for(const z of zones){state.waves[z.id]={wave:1,spawned:false,cleared:false};spawnZoneDecor(z);} spawnNaturalBarriers(); }
...
function update(dt){const p=state.player;state.feedback=state.feedback.filter(f=>(f.life-=dt)>0); let dx=(state.keys.has('d')?1:0)-(state.keys.has('a')?1:0),dy=(state.keys.has('s')?1:0)-(state.keys.has('w')?1:0); const m=Math.hypot(dx,dy)||1;dx/=m;dy/=m;
  ...
  for(const pr of state.projectiles){
    ...
    const nx=pr.x+pr.vx*dt,ny=pr.y+pr.vy*dt,hit=segmentObstacleHit(pr.x,pr.y,nx,ny,pr,{blockStumps:false});
    if(hit&&!pr.overStump){finishProjectileOnObstacle(pr,hit);if(pr.life<=0)continue;}
...
function drawLiveRangedPath(){
  const p=state.player,w=weaponDef();if(!p||!w||w.kind!=='charge')return false;const aim=Math.atan2(state.mouse.y-innerHeight/2,state.mouse.x-innerWidth/2),h=chargeLevel(state.mouse.held,w.id),sp=projectileLaunchSpeed(w,h);
  let x=p.x+Math.cos(aim)*30,y=p.y+Math.sin(aim)*30,vx=Math.cos(aim)*sp+(p.vx||0),vy=Math.sin(aim)*sp+(p.vy||0),life=w.id==='cannon'?1.35:w.id==='ballista'?1.8:2.4,dt=.06,points=[];
  for(let t=0;t<life;t+=dt){points.push(projectWorld(x,y));const drag=projectileDragFactor(Math.hypot(vx,vy),dt,false,projectileDragFor(w.id,state.slingMode,state.arrowMode));vx*=drag;vy*=drag;x+=vx*dt;y+=vy*dt;if(!getZone(x,y))break;}
  ctx.save();ctx.strokeStyle='rgba(255,255,255,.88)';ctx.fillStyle='rgba(20,30,45,.92)';ctx.lineWidth=2;ctx.setLineDash([3,8]);ctx.beginPath();points.forEach((q,i)=>i?ctx.lineTo(q.x,q.y):ctx.moveTo(q.x,q.y));ctx.stroke();ctx.setLineDash([]);for(let i=0;i<points.length;i+=3){const q=points[i];ctx.beginPath();ctx.arc(q.x,q.y,2.2,0,Math.PI*2);ctx.fill();}ctx.font='11px sans-serif';ctx.lineWidth=3;const label=projectWorld(x,y);ctx.strokeStyle='rgba(255,255,255,.8)';ctx.strokeText('expected path',label.x,label.y-12);ctx.fillText('expected path',label.x,label.y-12);ctx.restore();return true;
}
function drawLiveDamageReticle(){if(!state.damageView||state.mode!=='game'||!state.player)return;if(drawLiveRangedPath())return;drawOptimalReticle(liveDamageReticlePoint(),'strike');}
...
function hitChecks(){
  ...
  for(const pr of state.projectiles){
    ...
    if(pr.hostile){
      if(projectileHitsTarget(pr,player,2)){
    ...
    for(const e of state.enemies){
      ...
      if(projectileHitsTarget(pr,e,2)){
    ...
    for(const t of state.terrain){
      if(!t.solid||t.stump||pr.life<=0)continue;
      if(projectileHitsTarget(pr,t,4))finishProjectileOnObstacle(pr,{target:t,x:pr.x,y:pr.y,t:1});
    }
  }
...
function updateGrapeshotTest(dt){
  ...
  for(const pellet of g.shots){for(let step=0;step<3;step++){const h=dt/3;pellet.trail.push({x:pellet.x,y:pellet.y});if(pellet.trail.length>80)pellet.trail.shift();pellet.x+=pellet.vx*h;pellet.y+=pellet.vy*h;const localX=pellet.x-origin.x,localY=pellet.y-origin.y;if(!pellet.out&&pellet.x<=muzzle){if(localX<0){const d=Math.hypot(localX,localY)||1;if(d>r){const nx=localX/d,ny=localY/d,dot=pellet.vx*nx+pellet.vy*ny;pellet.vx-=dot*(2-g.absorb)*nx;pellet.vy-=dot*(2-g.absorb)*ny;pellet.x=origin.x+nx*r;pellet.y=origin.y+ny*r;}}else{if(localY<-r){pellet.y=origin.y-r;pellet.vy=Math.abs(pellet.vy)*(1-g.absorb);}if(localY>r){pellet.y=origin.y+r;pellet.vy=-Math.abs(pellet.vy)*(1-g.absorb);}}if(pellet.x>muzzle)pellet.out=true;}}}
}
...
function cheatWeaponUpgradeOptions(target){const bush=upgradeBushFor(target),stats=[...new Set([...bush.roots,...Object.keys(bush.reveals)])];return stats.map(stat=>upgradeOption(target,stat));}
function cheatUpgradeOptionsForZone(zone){
  if(zone.id==='armorer')return [{unlock:'shield',label:'unlock shield'},upgradeOption('shield','blockChance','merchant',16),upgradeOption('shield','blockAmount','merchant',16)];
  if(MERCHANT_ZONES.has(zone.id))return merchantOptions(zone,state.waves[zone.id]?.wave||1).filter(o=>!o.diamonds);
  if(zone.id==='smith')return [{unlock:'sword',label:'unlock sword'},...cheatWeaponUpgradeOptions('sword'),...cheatWeaponUpgradeOptions('pellet')];
  if(zone.id==='creek')return [{unlock:'sling',label:'unlock sling'},...cheatWeaponUpgradeOptions('sling'),...cheatWeaponUpgradeOptions('jar')];
  if(zone.id==='foundry')return FOUNDRY_UNLOCKS.map(unlock=>({unlock,label:`unlock ${unlock}`})).concat(cheatWeaponUpgradeOptions('cannon'));
  const opts=[];if(zone.unlock)opts.push({unlock:zone.unlock,label:`unlock ${zone.unlock}`});if(zone.element)opts.push({elementUnlock:zone.element,label:`unlock ${zone.element}`},...ELEMENT_UPGRADE_STATS[zone.element].map(stat=>upgradeOption(zone.element,stat,'element')));else if(zone.id!=='marches')opts.push(...cheatWeaponUpgradeOptions(rewardWeaponForZone(zone)));return opts;
}
function applyCheatUpgrade(option){
  if(!option)return;
  const p=state.player;if(option.unlock){p.unlocked.add(option.unlock);if(option.unlock==='horse')state.mount='horse';if(option.unlock==='shield')p.shield=true;if(weapons.some(w=>w.id===option.unlock))selectWeapon(option.unlock);return dbg(`[CHEAT] unlocked ${option.unlock}`);}
  if(option.elementUnlock){p.unlockedElements.add(option.elementUnlock);state.activeElement=option.elementUnlock;return dbg(`[CHEAT] unlocked ${option.elementUnlock}`);}
  const target=option.weapon||option.element||option.merchant;if(target&&option.stat){const m=metaFor(target);m[option.stat]++;if(target==='player'&&option.stat==='maxHp')p.maxHp++;p.hp=Math.min(p.maxHp,Math.max(p.hp,1));return dbg(`[CHEAT] ${target}.${option.stat} -> ${m[option.stat]}`);}
}
function cheatOptionByKey(key){const [zoneId,index]=key.split(':');const zone=zones.find(z=>z.id===zoneId);return zone?cheatUpgradeOptionsForZone(zone)[Number(index)]:null;}
function adjustCheatHealth(field,delta){const p=state.player;if(field==='max'){p.maxHp=Math.max(1,p.maxHp+delta);p.hp=Math.min(p.hp,p.maxHp);}else p.hp=Math.max(0,Math.min(p.maxHp,p.hp+delta));}
function adjustCheatMoney(delta){state.diamonds=Math.max(0,state.diamonds+delta);spawnSuitFeedback(state.player.x,state.player.y-42,'♦',delta,'#68d8ff',{negative:delta<0,mergeKey:'cheatMoney'});}
function cheatClearZone(){const zone=getZone(state.player.x,state.player.y);if(!zone)return;for(const e of state.enemies)if(e.zone===zone.id)e.hp=0;state.projectiles=state.projectiles.filter(pr=>!pr.hostile);completeWave(zone);dbg(`[CHEAT] cleared ${zone.name}`);}
function cheatStock(kind,amount=10){
  if(kind==='all'){for(const k of Object.keys(CONSUMABLE_BASE))cheatStock(k,10);return dbg('[CHEAT] stocked all consumables');}
  if(CONSUMABLE_HEALS[kind]){state.player.hp=Math.max(0,Math.min(state.player.maxHp,state.player.hp+CONSUMABLE_HEALS[kind]*amount));return;}
  if(kind==='antitoxin'){state.player.status.poison=0;return;}
  if(kind==='cloak'){state.player.cloaks=Math.max(0,(state.player.cloaks||0)+(amount>0?1:-1));return;}
  if(kind==='caltrops'){state.player.caltrops=Math.max(0,(state.player.caltrops||0)+amount);return;}
  if(kind==='decoy'){state.player.decoys=Math.max(0,(state.player.decoys||0)+amount);return;}
  if(['fire','ice','poison'].includes(kind)){state.elements[kind]=Math.max(0,state.elements[kind]+CONSUMABLE_QTY[kind]*amount);state.activeElement=kind;return;}
  if(kind in state.ammo)state.ammo[kind]=Math.max(0,state.ammo[kind]+CONSUMABLE_QTY[kind]*amount);
}
function cheatMenuHtml(){
  if(!state.cheatOpen)return '';
  const upgradeHtml=zones.map(zone=>{const opts=cheatUpgradeOptionsForZone(zone);if(!opts.length)return '';return `<details data-cheat-zone="${zone.id}"><summary>${zone.name}</summary>${opts.map((o,i)=>{const symbol=o.unlock||o.elementUnlock?'🔓':UPGRADE_SYMBOLS[o.stat]||'◆';return `<button type="button" data-cheat-upgrade="${zone.id}:${i}" data-tip="${o.label}">${symbol} ${o.label}</button>`;}).join(' ')}</details>`;}).join('');
  const consumables=Object.keys(CONSUMABLE_BASE).map(kind=>`<span class="qty-buy"><button data-cheat-stock="${kind}" data-cheat-stock-delta="-1">−</button><strong>${kind}</strong><button data-cheat-stock="${kind}" data-cheat-stock-delta="1">+</button></span>`).join(' ');
  return `<div class="shop-modal cheat-modal"><strong>Cheat Mode — paused</strong><button type="button" data-close-cheat="1">Close</button><div class="shop-section"><strong>Health & money</strong><br>Current <button data-cheat-health="hp" data-cheat-delta="-1">−</button><strong>${state.player.hp.toFixed(1)}</strong><button data-cheat-health="hp" data-cheat-delta="1">+</button> Max <button data-cheat-health="max" data-cheat-delta="-1">−</button><strong>${state.player.maxHp}</strong><button data-cheat-health="max" data-cheat-delta="1">+</button><br>Diamonds <button data-cheat-money="-100">−</button><strong>${state.diamonds}♦</strong><button data-cheat-money="100">+</button></div><div class="shop-section"><button type="button" data-cheat-clear="1">☠ Auto kill enemies and clear zone</button> <button type="button" data-cheat-stock="all">📦 Stock all consumables</button></div><div class="shop-section"><strong>Consumables</strong><br>${consumables}</div><div class="shop-section"><strong>All upgrades</strong>${upgradeHtml}</div></div>`;
}
function rememberCheatUiState(){const modal=ui.querySelector('.cheat-modal');return modal?{scrollTop:modal.scrollTop,open:[...modal.querySelectorAll('details[data-cheat-zone][open]')].map(d=>d.dataset.cheatZone)}:null;}
...
function projectileDrawAngle(pr){const base=pr.glyph==='triangle'?(pr.rotation||0):Math.atan2(pr.vy,pr.vx);return ['➵','➳','➶'].includes(pr.glyph)?base-0.08:base;}
function render(){updateHudFlashes();ctx.clearRect(0,0,innerWidth,innerHeight); drawHexBackground(); drawRiver(); drawSlingCreek(); drawDamagePreview(); drawMountainRims(); for(const f of coldSnowflakePositions()){const p=projectWorld(f.x,f.y);ctx.font='18px serif';ctx.fillStyle='#fff';ctx.fillText('❄️',p.x,p.y);}
 ...
 const z=getZone(state.player.x,state.player.y),cheatUiSnapshot=rememberCheatUiState();if(state.cheatOpen&&ui.querySelector('.cheat-modal')&&!state.cheatUiDirty){drawWeaponBar();return;} const dx=state.player.x-MAP_CENTER.x,dy=state.player.y-MAP_CENTER.y,rad=(Math.hypot(dx,dy)/(5*CELL*WORLD_SCALE)).toFixed(2),clock=wrap12((Math.atan2(dy,dx)*6/Math.PI)+3).toFixed(2);
...
function trig(){
  if(state.shopOpen||state.cheatOpen)return;
...
function loop(ts){if(!state.last)state.last=ts;const dt=Math.min(0.033,(ts-state.last)/1000);state.last=ts;if(state.mode==='game'){const p=state.player;p.lastX=p.x;p.lastY=p.y;if(!state.shopOpen&&!state.cheatOpen){state.t+=dt;update(dt);}render();}else if(state.mode==='glyph'){state.t+=dt;renderGlyphTest();}else if(state.mode==='grapeshot'){state.t+=dt;updateGrapeshotTest(dt);renderGrapeshotTest();}requestAnimationFrame(loop);}

function setMode(mode){state.mode=mode;menu.classList.toggle('hidden',mode!=='menu');ui.style.display=mode==='menu'?'none':'block';if(mode==='game'&&!state.player)resetWorld();if(mode==='glyph'||mode==='grapeshot')menu.classList.add('hidden');if(mode==='grapeshot'){ensureGrapeshotTest();if(!state.grapeshotTest.shots.length)fireGrapeshotTest();}}
menu.addEventListener('click',e=>{const mode=e.target.closest('[data-mode]')?.dataset.mode;if(mode)setMode(mode);});
function handleUiAction(e){const mode=e.target.closest('[data-mode]')?.dataset.mode; if(mode){e.preventDefault();e.stopPropagation();setMode(mode);return true;} const grapeAdjust=e.target.closest('[data-grape-adjust]');if(grapeAdjust){e.preventDefault();e.stopPropagation();adjustGrapeshotTest(grapeAdjust.dataset.grapeAdjust,Number(grapeAdjust.dataset.grapeDelta));return true;}if(e.target.closest('[data-grape-fire]')){e.preventDefault();e.stopPropagation();fireGrapeshotTest();return true;} if(e.target.closest('[data-close-cheat]')){e.preventDefault();e.stopPropagation();state.cheatOpen=false;state.cheatUiDirty=true;hideButtonTooltip();return true;}const cheatHealth=e.target.closest('[data-cheat-health]');if(cheatHealth){e.preventDefault();e.stopPropagation();adjustCheatHealth(cheatHealth.dataset.cheatHealth,Number(cheatHealth.dataset.cheatDelta));state.cheatUiDirty=true;return true;}const cheatMoney=e.target.closest('[data-cheat-money]');if(cheatMoney){e.preventDefault();e.stopPropagation();adjustCheatMoney(Number(cheatMoney.dataset.cheatMoney));state.cheatUiDirty=true;return true;}const cheatUpgrade=e.target.closest('[data-cheat-upgrade]');if(cheatUpgrade){e.preventDefault();e.stopPropagation();applyCheatUpgrade(cheatOptionByKey(cheatUpgrade.dataset.cheatUpgrade));state.cheatUiDirty=true;return true;}if(e.target.closest('[data-cheat-clear]')){e.preventDefault();e.stopPropagation();cheatClearZone();state.cheatUiDirty=true;return true;}const cheatStockButton=e.target.closest('[data-cheat-stock]');if(cheatStockButton){e.preventDefault();e.stopPropagation();cheatStockButton.dataset.cheatStockDelta?cheatStock(cheatStockButton.dataset.cheatStock,Number(cheatStockButton.dataset.cheatStockDelta)):cheatStock(cheatStockButton.dataset.cheatStock);state.cheatUiDirty=true;return true;} if(e.target.closest('[data-close-shop]')){e.preventDefault();e.stopPropagation();state.shopOpen=false;hideButtonTooltip();return true;}if(e.target.closest('[data-open-shop]')){e.preventDefault();e.stopPropagation();state.shopOpen=true;return true;} const glyph=e.target.closest('[data-glyph-weapon]')?.dataset.glyphWeapon; if(glyph){e.preventDefault();e.stopPropagation();state.glyphWeapon=glyph;return true;} const reward=e.target.closest('[data-reward-index]'); if(reward){e.preventDefault();e.stopPropagation();chooseReward(Number(reward.dataset.rewardIndex));return true;} const buy=e.target.closest('[data-buy]'); if(buy){e.preventDefault();e.stopPropagation();buyItem(buy.dataset.buy);return true;} const consumableAdjust=e.target.closest('[data-consumable-adjust]');if(consumableAdjust){e.preventDefault();e.stopPropagation();adjustConsumable(consumableAdjust.dataset.kind,Number(consumableAdjust.dataset.consumableAdjust));return true;} const financeAdjust=e.target.closest('[data-finance-adjust]');if(financeAdjust){e.preventDefault();e.stopPropagation();adjustFinance(financeAdjust.dataset.account,Number(financeAdjust.dataset.financeAdjust));return true;}const finance=e.target.closest('[data-finance]'); if(finance){e.preventDefault();e.stopPropagation();financeAction(finance.dataset.finance);return true;}const casinoAdjust=e.target.closest('[data-casino-adjust]');if(casinoAdjust){e.preventDefault();e.stopPropagation();adjustCasino(casinoAdjust.dataset.game,Number(casinoAdjust.dataset.casinoAdjust));return true;} const casino=e.target.closest('[data-casino]'); if(casino){e.preventDefault();e.stopPropagation();casinoAction(casino.dataset.casino);return true;} const el=e.target.closest('[data-element]'); if(el){e.preventDefault();e.stopPropagation();state.activeElement=state.activeElement===el.dataset.element?null:el.dataset.element;dbg(`[ELEMENT] applying ${state.activeElement||'none'}`);return true;} const sm=e.target.closest('[data-sling]'); if(sm){e.preventDefault();e.stopPropagation();state.slingMode=sm.dataset.sling;return true;} const ar=e.target.closest('[data-arrow]'); if(ar){e.preventDefault();e.stopPropagation();state.arrowMode=ar.dataset.arrow;return true;} const cm=e.target.closest('[data-cannon]'); if(cm){e.preventDefault();e.stopPropagation();state.cannonMode=cm.dataset.cannon;return true;} if(e.target.closest('[data-shield]')){e.preventDefault();e.stopPropagation();const p=state.player;if(!p.unlocked.has('shield'))dbg('[SHIELD] unlock shield at Armorer Court');else if(['bow','ballista','cannon'].includes(p.weapon))dbg('[SHIELD] cannot use shield with this weapon');else p.shield=!p.shield;return true;} if(e.target.closest('[data-enchant]')){e.preventDefault();e.stopPropagation();const p=state.player,el=state.activeElement,w=p.weapon;if(!['club','axe','sword'].includes(w))dbg('[ENCHANT] choose club, axe, or sword');else if(!el||!p.unlockedElements.has(el))dbg('[ENCHANT] choose an unlocked element');else if(Object.values(p.enchants).includes(el)&&p.enchants[w]!==el)dbg('[ENCHANT] that element is already bound');else{for(const k of Object.keys(p.enchants))if(p.enchants[k]===el)delete p.enchants[k];p.enchants[w]=el;dbg(`[ENCHANT] ${w} now carries ${el}`);}return true;} if(e.target.closest('[data-start-wave]')){e.preventDefault();e.stopPropagation();hideButtonTooltip();startNextWave();return true;} if(e.target.closest('[data-reset-world]')){e.preventDefault();e.stopPropagation();resetWorld();return true;} return false;}
ui.addEventListener('pointerdown',e=>{if(e.target.closest('summary'))return;handleUiAction(e)||e.stopPropagation();});
