...
function spawnWave(zone){ const wave=state.waves[zone.id]?.wave||1,radTier=zone.ring.min/RADIUS_UNIT,difficulty=1+radTier*0.55; state.waves[zone.id].spawned=true; const count=3+Math.min(10,Math.floor(wave*0.8+radTier)); const hp=7*difficulty*(1+wave*0.28); for(let i=0;i<count;i++){const pt=randomPointInZone(zone);state.enemies.push({x:pt.x,y:pt.y,hp,glyph:zone.enemy||'😠',zone:zone.id,minR:zone.ring.min,vx:0,vy:0,bounty:2+wave+Math.floor(radTier),hop:Math.random()*0.8});} dbg(`[WAVE] ${zone.name} wave ${wave} enemies=${count} hp=${hp.toFixed(1)} diff=${difficulty.toFixed(1)}`); }
...
function resolveEnemySpacing(){ const p=state.player; const pr=state.mount==='horse'&&p.unlocked.has('horse')?28:18, er=16; for(let i=0;i<state.enemies.length;i++){ const e=state.enemies[i]; for(let j=i+1;j<state.enemies.length;j++){ const o=state.enemies[j], dx=o.x-e.x, dy=o.y-e.y, d=Math.hypot(dx,dy)||1, min=er*2; if(d<min){ const push=(min-d)*0.5, nx=dx/d, ny=dy/d; e.x-=nx*push; e.y-=ny*push; o.x+=nx*push; o.y+=ny*push; } } const dx=p.x-e.x, dy=p.y-e.y, d=Math.hypot(dx,dy)||1, min=pr+er; if(d<min){ const nx=dx/d, ny=dy/d, push=min-d; e.x-=nx*push*0.65; e.y-=ny*push*0.65; p.x+=nx*push*0.35; p.y+=ny*push*0.35; p.vx+=nx*18; p.vy+=ny*18; p.hp=Math.max(0,p.hp-0.035); } } }
...
function update(dt){const p=state.player; let dx=(state.keys.has('d')?1:0)-(state.keys.has('a')?1:0),dy=(state.keys.has('s')?1:0)-(state.keys.has('w')?1:0); const m=Math.hypot(dx,dy)||1;dx/=m;dy/=m;
  ...
  for(const e of state.enemies){const bird=e.zone==='fletcher'; e.vx=(e.vx||0)*(bird?0.985:0.88); e.vy=(e.vy||0)*(bird?0.985:0.88); const ax=p.x-e.x,ay=p.y-e.y,d=Math.hypot(ax,ay)||1,ep=projectWorld(e.x,e.y),off=ep.x<-40||ep.x>innerWidth+40||ep.y<-40||ep.y>innerHeight+40,chaseMul=off?3.2:1; if(e.zone==='creek'){e.hop=(e.hop||0)-dt; if(e.hop<=0){let tx=ax/d,ty=ay/d; const frogs=state.enemies.filter(o=>o!==e&&o.zone==='creek'); let nearest=null,nd=Infinity; for(const o of frogs){const od=Math.hypot(o.x-e.x,o.y-e.y); if(od<nd){nd=od;nearest=o;}} if(nearest&&Math.random()<0.45){tx=(e.x-nearest.x)/(nd||1);ty=(e.y-nearest.y)/(nd||1);} e.vx+=tx*135*chaseMul; e.vy+=ty*135*chaseMul; e.hop=0.45+Math.random()*0.75;}} else { const accel=bird?130:12; e.vx += ax/d*accel*dt*chaseMul; e.vy += ay/d*accel*dt*chaseMul; if(!bird){e.x += ax/d*12*dt*chaseMul; e.y += ay/d*12*dt*chaseMul;}} e.x += e.vx*dt; e.y += e.vy*dt; const er=Math.hypot(e.x-MAP_CENTER.x,e.y-MAP_CENTER.y); if(e.minR&&er<e.minR){const sc=e.minR/Math.max(1,er);e.x=MAP_CENTER.x+(e.x-MAP_CENTER.x)*sc;e.y=MAP_CENTER.y+(e.y-MAP_CENTER.y)*sc;e.vx*=0.3;e.vy*=0.3;}} resolveEnemySpacing(); hitChecks(); if(p.hp<=0)resurrect();
