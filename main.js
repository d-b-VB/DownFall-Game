...
const BUILD_VERSION = 'v0.1.6 build 2026-05-30 00:00 UTC';
...
function update(dt){const p=state.player; let dx=(state.keys.has('d')?1:0)-(state.keys.has('a')?1:0),dy=(state.keys.has('s')?1:0)-(state.keys.has('w')?1:0); const m=Math.hypot(dx,dy)||1;dx/=m;dy/=m;
  ...
  for(const e of state.enemies){e.vx=(e.vx||0)*0.88; e.vy=(e.vy||0)*0.88; const ax=p.x-e.x,ay=p.y-e.y,d=Math.hypot(ax,ay)||1; if(e.zone==='creek'){e.hop=(e.hop||0)-dt; if(e.hop<=0){let tx=ax/d,ty=ay/d; const frogs=state.enemies.filter(o=>o!==e&&o.zone==='creek'); let nearest=null,nd=Infinity; for(const o of frogs){const od=Math.hypot(o.x-e.x,o.y-e.y); if(od<nd){nd=od;nearest=o;}} if(nearest&&Math.random()<0.45){tx=(e.x-nearest.x)/(nd||1);ty=(e.y-nearest.y)/(nd||1);} e.vx+=tx*135; e.vy+=ty*135; e.hop=0.45+Math.random()*0.75;}} else { e.vx += ax/d*12*dt; e.vy += ay/d*12*dt; e.x += ax/d*12*dt; e.y += ay/d*12*dt;} e.x += e.vx*dt; e.y += e.vy*dt;} resolveEnemySpacing(); hitChecks(); if(p.hp<=0)resurrect();
...
function drawHeldWeapon(w, aim){ const p=state.player, hold=Math.min(1,state.mouse.held/1.5), swingViz=(w.id==='club'||w.kind==='swing')?Math.sin((1-Math.max(0,p.swing))*Math.PI*2.4)*0.9*Math.max(0,p.swing):0, wob=(w.id==='bow'&&state.mouse.down)?Math.sin(state.t*20)*Math.max(0,state.mouse.held-0.7)*0.12:0; const gw=56,gh=44,pts=weaponGlyphPoints[w.id]||{},hp=cellPoint(pts.handleCell,gw,gh),tp=cellPoint(pts.tipCell,gw,gh),glyphAngle=Math.atan2(tp.y-hp.y,tp.x-hp.x); ctx.rotate(aim+swingViz+wob); ctx.translate(18,0); ctx.rotate(-glyphAngle); ctx.translate(-hp.x,-hp.y); drawWeaponVisual(w,gw,gh,(w.id==='bow'&&state.mouse.down)?hold:0.45); }
