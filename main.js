...
function update(dt){const p=state.player; let dx=(state.keys.has('d')?1:0)-(state.keys.has('a')?1:0),dy=(state.keys.has('s')?1:0)-(state.keys.has('w')?1:0); const m=Math.hypot(dx,dy)||1;dx/=m;dy/=m;
  ...
  for(const e of state.enemies){e.vx=(e.vx||0)*0.86; e.vy=(e.vy||0)*0.86; const ax=p.x-e.x,ay=p.y-e.y,d=Math.hypot(ax,ay)||1; e.vx += ax/d*22*dt; e.vy += ay/d*22*dt; e.x += e.vx*dt*18 + ax/d*24*dt; e.y += e.vy*dt*18 + ay/d*24*dt; if(d<20)p.hp=Math.max(0,p.hp-dt*2);} hitChecks();
