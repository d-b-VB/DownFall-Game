...
function merchantOptions(zone,wave){ const cost=upgradeCost(wave),cash={diamonds:6+wave*2,label:`take ${6+wave*2}♦`}; if(zone.id==='bank')return [{merchant:'bank',stat:'bounty',cost,label:`banker: +10% bounties (${cost}♦)`},cash]; if(zone.id==='tanner')return [{merchant:'foot',stat:'speed',cost,label:`tanner: +10% foot speed (${cost}♦)`},{merchant:'foot',stat:'accel',cost,label:`tanner: +10% foot acceleration (${cost}♦)`},cash]; if(zone.id==='inn')return [{merchant:'player',stat:'maxHp',cost:cost+4,label:`innkeeper: +1 max HP (${cost+4}♦)`},cash]; if(zone.id==='casino')return [{merchant:'casino',stat:'jackpot',cost,label:`casino: +10% hidden jackpot odds (${cost}♦)`},cash]; if(zone.id==='farrier')return [{weapon:'horse',stat:'accel',cost,label:`farrier: +10% horse acceleration (${cost}♦)`},{weapon:'horse',stat:'speed',cost,label:`farrier: +10% horse top speed (${cost}♦)`},cash]; if(zone.id==='armorer'){ if(!state.player.unlocked.has('shield'))return [{unlock:'shield',cost:unlockCost(zone),label:`armorer: unlock shield (${unlockCost(zone)}♦)`},cash]; return [{merchant:'shield',stat:'blockChance',cost,label:`armorer: +5% shield block chance (${cost}♦)`},{merchant:'shield',stat:'blockAmount',cost,label:`armorer: +5% shield reduction (${cost}♦)`},cash]; } return null;}
...
function buyItem(kind){
  const prices={arrows:1,jars:3,pellets:2,cannonballs:5,fire:4,ice:4,poison:4,antitoxin:5,fruit:1,meat:2,cheese:3,stew:4,cloak:4,caltrops:3,decoy:4},qty={arrows:6,jars:2,pellets:8,cannonballs:2,fire:2,ice:2,poison:2},heals={fruit:1,meat:2,cheese:3,stew:4};
  const here=getZone(state.player.x,state.player.y),z=(state.pendingReward&&here?.id===state.pendingReward.zone)?state.pendingReward.zone:state.currentZone,home={arrows:'fletcher',jars:'creek',pellets:'smith',cannonballs:'foundry',fruit:'orchard',meat:'axe',cheese:'pasture',stew:'inn',cloak:'tanner',caltrops:'farrier',decoy:'armorer',fire:'volcano',ice:'frost',poison:'swamp',antitoxin:'swamp'}[kind];
  if(home&&z!==home){dbg(`[SHOP] ${kind} is sold in ${home}`);return;}
  if(kind==='arrows'&&!state.player.unlocked.has('bow')){dbg('[SHOP] unlock bow before buying arrows');return;}
  if(kind==='jars'&&!state.player.unlocked.has('sling')){dbg('[SHOP] unlock sling before buying jars');return;}
  if(kind==='pellets'&&!state.player.unlocked.has('pellets')){dbg('[SHOP] unlock pellets before buying them');return;}
  if(kind==='cannonballs'&&!state.player.unlocked.has('cannon')){dbg('[SHOP] unlock cannon before buying cannonballs');return;}
  if(['fire','ice','poison'].includes(kind)&&!state.player.unlockedElements.has(kind)){dbg(`[SHOP] unlock ${kind} before buying charges`);return;}
  const cost=prices[kind];if(cost===undefined)return;if(state.diamonds<cost){dbg(`[SHOP] need ${cost}♦ for ${kind}`);return;}state.diamonds-=cost;
  if(heals[kind]){const before=state.player.hp;state.player.hp=Math.min(state.player.maxHp,state.player.hp+heals[kind]);dbg(`[HEAL] ${kind} +${(state.player.hp-before).toFixed(1)} HP`);return;}
  if(kind==='antitoxin'){state.player.status.poison=0;dbg('[SHOP] anti-toxin cleared poison');return;}
  if(kind==='cloak'){state.player.cloaks=(state.player.cloaks||0)+1;dbg('[SHOP] elemental cloak +1');return;}
  if(kind==='caltrops'){state.player.caltrops=(state.player.caltrops||0)+1;dbg('[SHOP] caltrops +1');return;}
  if(kind==='decoy'){state.player.decoys=(state.player.decoys||0)+1;dbg('[SHOP] armor-stand decoy +1');return;}
  if(['fire','ice','poison'].includes(kind)){state.elements[kind]+=qty[kind];state.activeElement=kind;dbg(`[SHOP] bought ${qty[kind]} ${kind} charges`);return;}
...
function financeAction(kind){
  ...
  if(kind==='deposit'){if(state.diamonds<5)return dbg('[BANK] need 5♦ to deposit');state.diamonds-=5;f.savings+=5;dbg(`[BANK] savings ${f.savings}♦`);}
  if(kind==='withdraw'){const n=Math.min(5,f.savings);f.savings-=n;state.diamonds+=n;dbg(`[BANK] withdrew ${n}♦`);}
  if(kind==='loan'){state.diamonds+=10;f.debt+=13;dbg(`[BANK] borrowed 10♦; total repayment ${f.debt}♦`);}
  if(kind==='repay'){const n=Math.min(5,state.diamonds,f.debt);state.diamonds-=n;f.debt-=n;dbg(`[BANK] repaid ${n}♦; debt ${f.debt}♦`);}
}
function casinoAction(stake){
  if(getZone(state.player.x,state.player.y)?.id!=='casino'||state.pendingReward?.zone!=='casino'||state.diamonds<stake)return;
  state.diamonds-=stake;const chance=stake===1?0.4:1/6,payout=stake===1?2:25;if(Math.random()<chance){state.diamonds+=payout;dbg(`[CASINO] won ${payout}♦`);}else dbg(`[CASINO] lost ${stake}♦`);
}
function shopConsumables(zoneId){
  const buttons={orchard:[['fruit','🍎 fruit +1 HP · 1♦']],axe:[['meat','🍖 meat +2 HP · 2♦']],pasture:[['cheese','🧀 cheese +3 HP · 3♦']],inn:[['stew','🍲 stew +4 HP · 4♦']],creek:[['jars','🏺 2 jars · 3♦']],fletcher:[['arrows','➳ 6 arrows · 1♦']],smith:[['pellets','● 8 pellets · 2♦']],foundry:[['cannonballs','⚫ 2 cannonballs · 5♦']],tanner:[['cloak','🧥 elemental cloak · 4♦']],farrier:[['caltrops','✣ caltrops · 3♦']],armorer:[['decoy','🛡️ armor decoy · 4♦']],volcano:[['fire','🔥 2 fire charges · 4♦']],frost:[['ice','❄️ 2 ice charges · 4♦']],swamp:[['poison','☠️ 2 poison charges · 4♦'],['antitoxin','🧪 anti-toxin · 5♦']]}[zoneId]||[];
  let html=buttons.map(([kind,label])=>`<button type="button" data-buy="${kind}">${label}</button>`).join(' ');
  if(zoneId==='bank')html=`Savings ${state.finance.savings}♦ · Debt ${state.finance.debt}♦<br><button data-finance="deposit">deposit 5♦</button><button data-finance="withdraw">withdraw up to 5♦</button><button data-finance="loan">borrow 10♦ (repay 13♦)</button><button data-finance="repay">repay up to 5♦</button>`;
  if(zoneId==='casino')html=`<button data-casino="1">wager 1♦ · pays 2♦</button><button data-casino="5">wager 5♦ · pays 25♦</button>`;
...
function update(dt){const p=state.player; let dx=(state.keys.has('d')?1:0)-(state.keys.has('a')?1:0),dy=(state.keys.has('s')?1:0)-(state.keys.has('w')?1:0); const m=Math.hypot(dx,dy)||1;dx/=m;dy/=m;
  ...
  for(const e of state.enemies){
    ...
    const oldX=e.x,oldY=e.y,ax=p.x-e.x,ay=p.y-e.y,d=Math.hypot(ax,ay)||1,profile=enemyMotionProfile(e),ep=projectWorld(e.x,e.y),off=ep.x<-40||ep.x>innerWidth+40||ep.y<-40||ep.y>innerHeight+40;
...
function drawWeaponBar(){
  const available=weapons.filter(w=>state.player.unlocked.has(w.id)),cell=54,total=available.length*cell,x0=innerWidth/2-total/2,y=innerHeight-38;
  ctx.save();ctx.textAlign='center';ctx.textBaseline='middle';ctx.font='12px sans-serif';
  available.forEach((weapon,i)=>{const x=x0+i*cell+cell/2,active=weapon.id===state.player.weapon;ctx.fillStyle=active?'rgba(245,205,75,.9)':'rgba(0,0,0,.62)';ctx.strokeStyle=active?'#fff3a1':'#ffffff55';ctx.lineWidth=active?3:1;ctx.beginPath();ctx.roundRect(x-23,y-25,46,48,8);ctx.fill();ctx.stroke();ctx.fillStyle='#fff';ctx.fillText(String(weapons.indexOf(weapon)+1),x-16,y-16);ctx.font='22px serif';ctx.fillText(weapon.id==='club'?'┃':weapon.glyph,x,y+3);ctx.font='12px sans-serif';});ctx.restore();
}
function render(){ctx.clearRect(0,0,innerWidth,innerHeight); drawHexBackground(); drawRiver(); drawSlingCreek(); for(const f of coldSnowflakePositions()){const p=projectWorld(f.x,f.y);ctx.font='18px serif';ctx.fillStyle='#fff';ctx.fillText('❄️',p.x,p.y);}
 ...
 ui.innerHTML=`<strong>${BUILD_VERSION}</strong><br>HP ${state.player.hp.toFixed(1)}/${state.player.maxHp} · ♦ ${state.diamonds}<br>Zone: ${z?z.name:'Boundary'} · R ${rad} · ${clock}:00<br>Weapon: ${w.id} · Mount: ${state.mount}<br>Ammo: ➳ ${state.ammo.arrows} · 🏺 ${state.ammo.jars} · ● ${state.ammo.pellets} · ⚫ ${state.ammo.cannonballs}${modeBits.length?`<br>${modeBits.join('<br>')}`:''}${shopHtml}<br><button type="button" data-reset-world="1">Reset World</button>`;
 drawWeaponBar(); }
