...
const BUILD_VERSION = 'v0.2.5 build 2026-06-01 00:00 UTC';
...
function resolveEnemySpacing(){ const p=state.player; const pr=state.mount==='horse'&&p.unlocked.has('horse')?28:18, er=16; for(let i=0;i<state.enemies.length;i++){ const e=state.enemies[i]; for(let j=i+1;j<state.enemies.length;j++){ const o=state.enemies[j], dx=o.x-e.x, dy=o.y-e.y, d=Math.hypot(dx,dy)||1, min=er*2; if(d<min){ const push=(min-d)*0.5, nx=dx/d, ny=dy/d; e.x-=nx*push; e.y-=ny*push; o.x+=nx*push; o.y+=ny*push; } } const dx=p.x-e.x, dy=p.y-e.y, d=Math.hypot(dx,dy)||1, min=pr+er; if(d<min){ const nx=dx/d, ny=dy/d, push=min-d; e.x-=nx*push*0.65; e.y-=ny*push*0.65; p.x+=nx*push*0.35; p.y+=ny*push*0.35; p.vx+=nx*18; p.vy+=ny*18; p.hp=Math.max(0,p.hp-0.045*(e.power||1)); if(e.zone==='swamp')addStatus(p,'poison',0.02*(e.power||1)); } } }
...
function resetWorld(){const s={x:MAP_CENTER.x,y:MAP_CENTER.y};state.player={x:s.x,y:s.y,vx:0,vy:0,hp:14,maxHp:14,status:{},weapon:'club',unlocked:new Set(['club']),swing:0,cannonCooldown:0,lastX:s.x,lastY:s.y}; state.mount='foot'; state.ammo={arrows:0,jars:0}; state.elements={fire:0,ice:0,poison:0}; state.activeElement=null; state.projectiles=[];state.enemies=[];state.terrain=[];state.waves={};state.pendingReward=null;state.offerNpc=null;state.currentZone=null; for(const z of zones){state.waves[z.id]={wave:1,spawned:false,cleared:false};spawnZoneDecor(z);} }

// Waterways use 12-hour clock bearings: cold river flows from Frosty Mountain along the Tundra/Fletcher boundary through Marsh into Swamp; warm creek starts at Volcano, wiggles through Sling Creekside, then joins Marsh/Swamp.
const coldRiverPath=[{h:10.35,r:3.95},{h:11.35,r:3.35},{h:0.15,r:3.05},{h:1.15,r:3.05},{h:2.45,r:3.18},{h:3.05,r:3.75},{h:3.35,r:4.55}];
const warmCreekPath=[{h:7.35,r:3.95},{h:8.05,r:3.1},{h:8.95,r:2.25},{h:10.25,r:1.45},{h:11.55,r:1.2},{h:0.8,r:1.55},{h:2.2,r:2.7},{h:3.0,r:3.38},{h:3.5,r:4.52}];
...
function update(dt){const p=state.player; let dx=(state.keys.has('d')?1:0)-(state.keys.has('a')?1:0),dy=(state.keys.has('s')?1:0)-(state.keys.has('w')?1:0); const m=Math.hypot(dx,dy)||1;dx/=m;dy/=m;
  const zone=getZone(p.x,p.y), flow=riverPushAt(p.x,p.y); state.lastWater=flow.type?`${flow.type} d=${flow.d.toFixed(0)} push=${Math.hypot(flow.x,flow.y).toFixed(0)}`:'dry'; if(flow.damage)p.hp=Math.max(0,p.hp-flow.damage*dt); tickStatus(p,dt,true); if(zone?.id!==state.currentZone){state.currentZone=zone?.id||null; if(zone)dbg(`[ZONE] entered ${zone.name} wave=${state.waves[zone.id]?.wave||1} spawned=${!!state.waves[zone.id]?.spawned}`);} const horseEq=state.mount==='horse'&&p.unlocked.has('horse'); const horse=metaFor('horse'); const speed=horseEq?2.25*(1+horse.speed*0.1):1,accel=horseEq?4.8*(1+horse.accel*0.1):7.5;
  ...
  for(const e of state.enemies){tickStatus(e,dt,false); const bird=e.zone==='fletcher', em=(e.power||1); e.vx=(e.vx||0)*(bird?0.985:0.88); e.vy=(e.vy||0)*(bird?0.985:0.88); const ax=p.x-e.x,ay=p.y-e.y,d=Math.hypot(ax,ay)||1,ep=projectWorld(e.x,e.y),off=ep.x<-40||ep.x>innerWidth+40||ep.y<-40||ep.y>innerHeight+40,chaseMul=off?3.2:1,iceMul=e.status?.ice>0?Math.max(0.15,1-0.35*(1+(metaFor('ice').slow||0)*0.1)):1; if(e.zone==='creek'){e.hop=(e.hop||0)-dt; if(e.hop<=0){let tx=ax/d,ty=ay/d; const frogs=state.enemies.filter(o=>o!==e&&o.zone==='creek'); let nearest=null,nd=Infinity; for(const o of frogs){const od=Math.hypot(o.x-e.x,o.y-e.y); if(od<nd){nd=od;nearest=o;}} if(nearest&&Math.random()<0.45){tx=(e.x-nearest.x)/(nd||1);ty=(e.y-nearest.y)/(nd||1);} e.vx+=tx*135*chaseMul*iceMul; e.vy+=ty*135*chaseMul*iceMul; e.hop=0.45+Math.random()*0.75;}} else { const accel=(bird?130:12)*Math.min(2.6,em); e.vx += ax/d*accel*dt*chaseMul*iceMul; e.vy += ay/d*accel*dt*chaseMul*iceMul; if(!bird){e.x += ax/d*12*dt*chaseMul*iceMul*Math.min(2,em); e.y += ay/d*12*dt*chaseMul*iceMul*Math.min(2,em);}} const ef=waterPushAt(e.x,e.y); e.x += (e.vx+ef.x)*dt; e.y += (e.vy+ef.y)*dt; if(ef.damage)e.hp-=ef.damage*dt*0.35; const er=Math.hypot(e.x-MAP_CENTER.x,e.y-MAP_CENTER.y); if(e.minR&&er<e.minR){const sc=e.minR/Math.max(1,er);e.x=MAP_CENTER.x+(e.x-MAP_CENTER.x)*sc;e.y=MAP_CENTER.y+(e.y-MAP_CENTER.y)*sc;e.vx*=0.3;e.vy*=0.3;} if(e.zone==='creek'&&getZone(e.x,e.y)?.id!=='creek'){const cz=zones.find(z=>z.id==='creek'),pt=randomPointInZone(cz);e.x=pt.x;e.y=pt.y;e.vx=0;e.vy=0;}} resolveEnemySpacing(); hitChecks(); if(p.hp<=0){resurrect();return;}
...
function render(){ctx.clearRect(0,0,innerWidth,innerHeight); drawHexBackground(); drawRiver(); drawSlingCreek();
 ...
 const tiers=metaFor(w.id); const showReward=state.pendingReward&&z?.id===state.pendingReward.zone; const specialist=z?.element?`<br>${ZONE_NPCS[z.id]} ${z.id==='swamp'?'Auntie Toxin':z.id==='frost'?'Alchemist':'Wizard'}: <button type="button" data-buy="${z.element}">buy ${z.element} 4♦</button>${z.id==='swamp'?' <button type="button" data-buy="antitoxin">anti-toxin 5♦</button><button type="button" data-buy="poisonResist">resist +10% 10♦</button><button type="button" data-buy="poisonUpgrade">poison +10% 12♦</button>':''}`:''; const elemBtns=`<br>Apply: ${['fire','ice','poison'].map(e=>`<button type="button" data-element="${e}">${e}${state.activeElement===e?'*':''} ${state.elements[e]}</button>`).join(' ')}`; const shopHtml=(showReward?`<br>${state.pendingReward.options.map((o,i)=>`<button type="button" data-reward-index="${i}" ${state.pendingReward.purchased?'disabled':''}>${o.label}</button>`).join(' ')} ${state.pendingReward.zone==='creek'?'<button type="button" data-buy="jars">buy jars 3♦</button>':''}${state.pendingReward.zone==='fletcher'?'<button type="button" data-buy="arrows">buy arrows 1♦</button>':''}${state.pendingReward.purchased?'<button type="button" data-start-wave="1">Start next wave</button>':''}`:'')+specialist+elemBtns; ui.innerHTML=`HP ${state.player.hp.toFixed(1)} · ♦ ${state.diamonds}<br>Zone: ${z?z.name:'Boundary'}<br>R: ${rad} · Clock12: ${clock}<br>Weapon: ${w.id} (${tiers.speed}/${tiers.range}/${tiers.damage}/${tiers.knockback})<br>Ammo: arrows ${state.ammo.arrows} · jars ${state.ammo.jars}<br>Status: poison ${state.player.status.poison||0} · fire ${state.player.status.fire?.toFixed(1)||0}<br>Mount: ${state.mount}<br>Unlocked: ${[...state.player.unlocked].join(', ')}${shopHtml}<br><button type="button" data-reset-world="1">Reset World</button>`;
  ...
  const lines=[`run=${state.run} diamonds=${state.diamonds} pending=${state.pendingReward?state.pendingReward.zone:'none'}`,`px=${state.player.x.toFixed(1)} py=${state.player.y.toFixed(1)} vx=${state.player.vx.toFixed(2)} vy=${state.player.vy.toFixed(2)}`,`water=${state.lastWater||'dry'} · cold river: frost→tundra/fletcher→marsh→swamp · warm creek: volcano→creek→marsh/swamp`,`mouse=(${state.mouse.x.toFixed(0)},${state.mouse.y.toFixed(0)}) hold=${state.mouse.held.toFixed(2)}`,...state.debug];
