...
const SMITH_UNLOCKS = ['sword','pellets'];
...
function merchantOptions(zone,wave){ const cost=upgradeCost(wave),cash={diamonds:6+wave*2,label:`take ${6+wave*2}♦`}; if(zone.id==='bank')return [{merchant:'bank',stat:'bounty',cost,label:`banker: +10% bounties (${cost}♦)`},cash]; if(zone.id==='tanner')return [{merchant:'foot',stat:'speed',cost,label:`tanner: +10% foot speed (${cost}♦)`},{merchant:'foot',stat:'accel',cost,label:`tanner: +10% foot acceleration (${cost}♦)`},cash]; if(zone.id==='inn')return [{merchant:'player',stat:'maxHp',cost:cost+4,label:`innkeeper: +1 max HP (${cost+4}♦)`},{heal:4,cost:4,label:'buy stew: heal 4 HP (4♦)'},cash]; if(zone.id==='casino')return [{merchant:'casino',stat:'jackpot',cost,label:`casino: +10% hidden jackpot odds (${cost}♦)`},cash]; if(zone.id==='farrier')return [{weapon:'horse',stat:'accel',cost,label:`farrier: +10% horse acceleration (${cost}♦)`},{weapon:'horse',stat:'speed',cost,label:`farrier: +10% horse top speed (${cost}♦)`},cash]; if(zone.id==='armorer'){ if(!state.player.unlocked.has('shield'))return [{unlock:'shield',cost:unlockCost(zone),label:`armorer: unlock shield (${unlockCost(zone)}♦)`},cash]; return [{merchant:'shield',stat:'blockChance',cost,label:`armorer: +5% shield block chance (${cost}♦)`},{merchant:'shield',stat:'blockAmount',cost,label:`armorer: +5% shield reduction (${cost}♦)`},cash]; } return null;}
...
function spawnWave(zone){ const wave=state.waves[zone.id]?.wave||1,radTier=zone.ring.min/RADIUS_UNIT,difficulty=Math.pow(1.65,radTier),enemyPower=1.25*Math.pow(2.15,radTier)*Math.pow(1.1,wave*3); state.waves[zone.id].spawned=true; const count=6+Math.min(24,Math.floor(wave*1.25+radTier*2.1)); let hp=5*difficulty*(1+wave*0.08); if(zone.id==='fletcher')hp*=0.55; for(let i=0;i<count;i++){const pt=spawnPointOuterEdge(zone,i,count),v=enemyVariant(zone,wave,i);state.enemies.push({x:pt.x,y:pt.y,hp:hp*v.hpMul,maxHp:hp*v.hpMul,status:{},glyph:v.glyph,kind:v.kind,weapon:v.weapon,zone:zone.id,minR:zone.ring.min,vx:0,vy:0,bounty:2+wave+Math.floor(radTier),hop:Math.random()*0.8,power:enemyPower*v.powMul});} dbg(`[WAVE] ${zone.name} wave ${wave} enemies=${count} hp=${hp.toFixed(1)} power=${enemyPower.toFixed(2)}`); }
...
function resolveEnemySpacing(dt){ const p=state.player; const pr=state.mount==='horse'&&p.unlocked.has('horse')?28:18, er=16; for(let i=0;i<state.enemies.length;i++){ const e=state.enemies[i]; for(let j=i+1;j<state.enemies.length;j++){ const o=state.enemies[j], dx=o.x-e.x, dy=o.y-e.y, d=Math.hypot(dx,dy)||1, min=er*2; if(d<min){ const push=(min-d)*0.5, nx=dx/d, ny=dy/d; e.x-=nx*push; e.y-=ny*push; o.x+=nx*push; o.y+=ny*push; } } const dx=p.x-e.x, dy=p.y-e.y, d=Math.hypot(dx,dy)||1, min=pr+er; if(d<min){ const nx=dx/d, ny=dy/d, push=min-d; e.x-=nx*push*0.65; e.y-=ny*push*0.65; p.x+=nx*push*0.35; p.y+=ny*push*0.35; p.vx+=nx*18; p.vy+=ny*18; let hit=(0.7+0.55*Math.sqrt(e.power||1))*dt*(e.kind==='turtle'||e.kind==='turkey'?1.7:e.kind==='eagle'?1+Math.min(1.5,Math.hypot(e.vx||0,e.vy||0)/260):1); if(p.shield){const sh=metaFor('shield'),chance=Math.min(0.75,0.3+(sh.blockChance||0)*0.05),block=Math.min(0.8,0.3+(sh.blockAmount||0)*0.05); if(Math.random()<chance){hit*=1-block;p.vx+=nx*22;p.vy+=ny*22;e.vx-=nx*22;e.vy-=ny*22;dbg(`[SHIELD] blocked ${(block*100).toFixed(0)}% contact damage`);}} p.hp=Math.max(0,p.hp-hit); if(['swamp','axe','marsh'].includes(e.zone))addStatus(p,'poison',0.02*(e.power||1)); } } }
...
function applyEnvironmentalHazards(o,dt){
  ...
  for(const h of state.hazards){if(h.stump)continue;const dx=o.x-h.x,dy=o.y-h.y,d=Math.hypot(dx,dy);if(d>44)continue;
    if(h.hazard==='fire'){o.hp=Math.max(0,o.hp-2.4*dt);if(o.hazardCooldown<=0){addStatus(o,'fire',0.55);o.hazardCooldown=0.65;}}
    if(h.hazard==='cactus'){o.hp=Math.max(0,o.hp-1.2*dt);const k=34/(d||1);o.vx=(o.vx||0)+dx*k;o.vy=(o.vy||0)+dy*k;}
  }
}

function resetWorld(){const s={x:MAP_CENTER.x,y:MAP_CENTER.y};state.player={x:s.x,y:s.y,vx:0,vy:0,hp:14+(metaFor('player').maxHp||0),maxHp:14+(metaFor('player').maxHp||0),status:{},weapon:'club',unlocked:new Set(['club']),unlockedElements:new Set(),shield:false,enchants:{},swing:0,cannonCooldown:0,lastX:s.x,lastY:s.y}; state.mount='foot'; state.ammo={arrows:0,jars:0,pellets:0,cannonballs:0}; state.slingMode='rock'; state.arrowMode='regular'; state.elements={fire:0,ice:0,poison:0}; state.activeElement=null; state.projectiles=[];state.enemies=[];state.terrain=[];state.hazards=[];state.waves={};state.pendingReward=null;state.offerNpc=null;state.currentZone=null; for(const z of zones){state.waves[z.id]={wave:1,spawned:false,cleared:false};spawnZoneDecor(z);} spawnNaturalBarriers(); }
...
function applyColdSnowflakes(o,dt,flakes=coldSnowflakePositions()){o.snowCooldown=Math.max(0,(o.snowCooldown||0)-dt);for(const f of flakes){if(Math.hypot(o.x-f.x,o.y-f.y)<30&&o.snowCooldown<=0){o.hp=Math.max(0,o.hp-0.7);addStatus(o,'ice',0.65);o.snowCooldown=0.75;break;}}}
...
function enemyMotionProfile(e){
  ...
  const profiles={
    orchard:{top:112,cruise:0.9,fast:520,slow:12},
    creek:{top:178,cruise:0.82,fast:760,slow:25,hop:true},
    pasture:{top:192,cruise:0.88,fast:760,slow:18},
    pawn:{top:188,cruise:0.9,fast:690,slow:14},
    crow:{top:238,cruise:0.82,fast:560,slow:10},
    turkey:{top:142,cruise:0.9,fast:420,slow:8},
    eagle:{top:282,cruise:0.8,fast:500,slow:9},
    squirrel:{top:210,cruise:0.86,fast:820,slow:22},
    turtle:{top:104,cruise:0.92,fast:300,slow:7},
    deer:{top:266,cruise:0.84,fast:620,slow:12},
    rabbit:{top:248,cruise:0.84,fast:920,slow:24},
  };
  const chosen=profiles[e.kind]||profiles[e.zone];
  return chosen||{top:ringTop,cruise:0.88,fast:ringTop*3.4,slow:ringTop*0.04};
}
function steerEnemy(e,dirX,dirY,top,profile,dt,accelMul=1){
  const len=Math.hypot(dirX,dirY)||1;dirX/=len;dirY/=len;
  const speed=Math.hypot(e.vx||0,e.vy||0),cruise=top*profile.cruise;
  const accel=(speed<cruise?profile.fast:profile.slow)*accelMul;
  const targetX=dirX*top,targetY=dirY*top,dvx=targetX-(e.vx||0),dvy=targetY-(e.vy||0),delta=Math.hypot(dvx,dvy)||1,step=Math.min(delta,accel*dt);
  e.vx=(e.vx||0)+dvx/delta*step;e.vy=(e.vy||0)+dvy/delta*step;
}
...
function update(dt){const p=state.player; let dx=(state.keys.has('d')?1:0)-(state.keys.has('a')?1:0),dy=(state.keys.has('s')?1:0)-(state.keys.has('w')?1:0); const m=Math.hypot(dx,dy)||1;dx/=m;dy/=m;
  ...
  for(const e of state.enemies){
    ...
    const ax=p.x-e.x,ay=p.y-e.y,d=Math.hypot(ax,ay)||1,profile=enemyMotionProfile(e),ep=projectWorld(e.x,e.y),off=ep.x<-40||ep.x>innerWidth+40||ep.y<-40||ep.y>innerHeight+40;
    let dirX=ax/d,dirY=ay/d,top=profile.top,accelMul=off?1.55:1;
    ...
    if(obstacle){e.vx*=-0.25;e.vy*=-0.25;}else{e.x=nx;e.y=ny;}
  ...
  } resolveEnemySpacing(dt); hitChecks(); if(p.hp<=0){resurrect();return;}
...
function render(){ctx.clearRect(0,0,innerWidth,innerHeight); drawHexBackground(); drawRiver(); drawSlingCreek(); for(const f of coldSnowflakePositions()){const p=projectWorld(f.x,f.y);ctx.font='18px serif';ctx.fillStyle='#fff';ctx.fillText('❄️',p.x,p.y);}
 ...
 const tiers=metaFor(w.id); const showReward=state.pendingReward&&z?.id===state.pendingReward.zone; const specialist=z?.element?`<br>${ZONE_NPCS[z.id]} ${z.id==='swamp'?'Auntie Toxin':z.id==='frost'?'Alchemist':'Wizard'}: <button type="button" data-buy="${z.element}">buy ${z.element} 4♦</button>${z.id==='swamp'?' <button type="button" data-buy="antitoxin">anti-toxin 5♦</button>':''}`:''; const modeBits=[]; if(['bow','sling','club','axe','sword'].includes(w.id))modeBits.push(`Apply: ${['fire','ice','poison'].map(e=>`<button type="button" data-element="${e}">${e}${state.activeElement===e?'*':''} ${state.elements[e]}</button>`).join(' ')}`); if(['club','axe','sword'].includes(w.id))modeBits.push('<button type="button" data-enchant="1">enchant held</button>'); if(w.id==='sling')modeBits.push(`Sling: ${['rock','pellet','jar','elementJar'].map(m=>`<button type="button" data-sling="${m}">${m}${state.slingMode===m?'*':''}</button>`).join(' ')}`); if(w.id==='bow')modeBits.push(`Arrows: ${['regular','element'].map(m=>`<button type="button" data-arrow="${m}">${m}${state.arrowMode===m?'*':''}</button>`).join(' ')}`); if(w.id==='cannon')modeBits.push(`Cannon: ${['ball','grape'].map(m=>`<button type="button" data-cannon="${m}">${m}${state.cannonMode===m?'*':''}</button>`).join(' ')}`); if(state.player.unlocked.has('shield')&&!['bow','ballista','cannon'].includes(w.id))modeBits.push('<button type="button" data-shield="1">shield (Q)</button>'); const elemBtns=modeBits.length?`<br>${modeBits.join('<br>')}`:''; const shopHtml=(showReward?`<br>${state.pendingReward.options.map((o,i)=>`<button type="button" data-reward-index="${i}" ${state.pendingReward.purchased?'disabled':''}>${o.label}</button>`).join(' ')} ${state.pendingReward.zone==='creek'?'<button type="button" data-buy="jars">buy jars 3♦</button>':''}${state.pendingReward.zone==='fletcher'?'<button type="button" data-buy="arrows">buy arrows 1♦</button>':''}${state.pendingReward.zone==='smith'?'<button type="button" data-buy="pellets">buy pellets 2♦</button>':''}${state.pendingReward.zone==='foundry'?'<button type="button" data-buy="cannonballs">buy cannonballs 5♦</button>':''}${state.pendingReward.purchased?'<button type="button" data-start-wave="1">Start next wave</button>':''}`:'')+specialist+elemBtns; ui.innerHTML=`HP ${state.player.hp.toFixed(1)} · ♦ ${state.diamonds}<br>Zone: ${z?z.name:'Boundary'}<br>R: ${rad} · Clock12: ${clock}<br>Weapon: ${w.id} (${tiers.speed}/${tiers.range}/${tiers.damage}/${tiers.knockback})<br>Ammo: arrows ${state.ammo.arrows} · jars ${state.ammo.jars} · pellets ${state.ammo.pellets} · cannonballs ${state.ammo.cannonballs}<br>Modes: ${[w.id==='sling'?`sling ${state.slingMode}`:'',w.id==='bow'?`arrows ${state.arrowMode}`:'',w.id==='cannon'?`cannon ${state.cannonMode}`:'',state.player.unlocked.has('shield')&&!['bow','ballista','cannon'].includes(w.id)?`shield ${state.player.shield?'on':'off'}`:''].filter(Boolean).join(' · ')||'none'}<br>Status: poison ${state.player.status.poison||0} · fire ${state.player.status.fire?.toFixed(1)||0}<br>Mount: ${state.mount}<br>Unlocked: ${[...state.player.unlocked].join(', ')}${shopHtml}<br><button type="button" data-reset-world="1">Reset World</button>`;
  ...
  const lines=[`run=${state.run} diamonds=${state.diamonds} pending=${state.pendingReward?state.pendingReward.zone:'none'}`,`px=${state.player.x.toFixed(1)} py=${state.player.y.toFixed(1)} vx=${state.player.vx.toFixed(2)} vy=${state.player.vy.toFixed(2)}`,`water=${state.lastWater||'dry'} · cold: frost→merchant north edge→tundra/sawmill→marsh→swamp · warm: volcano→smith/pasture→creek oxbow→H3 boundary→swamp`,`mouse=(${state.mouse.x.toFixed(0)},${state.mouse.y.toFixed(0)}) hold=${state.mouse.held.toFixed(2)}`,...state.debug];
