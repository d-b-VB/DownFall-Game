...
const BUILD_VERSION = 'v0.2.1 build 2026-05-31 00:00 UTC';
...
function resurrect(){ dbg(`[DOWNFALL] run ${state.run} ended; upgrades persist, waves reset`); state.run++; state.player.x=MAP_CENTER.x; state.player.y=MAP_CENTER.y; state.player.vx=0; state.player.vy=0; state.player.hp=14; state.player.weapon='club'; state.player.unlocked=new Set(['club']); state.mount='foot'; state.enemies=[]; state.projectiles=[]; state.ammo={arrows:0,jars:0}; state.pendingReward=null; state.offerNpc=null; state.currentZone=null; for(const z of zones) state.waves[z.id]={wave:1,spawned:false,cleared:false}; }
...
function spawnWave(zone){ const wave=state.waves[zone.id]?.wave||1,radTier=zone.ring.min/RADIUS_UNIT,difficulty=1+radTier*0.7,enemyPower=1.25*Math.pow(1.1,wave*3+radTier*2); state.waves[zone.id].spawned=true; const count=4+Math.min(12,Math.floor(wave+radTier*1.2)); const hp=7*difficulty*enemyPower; for(let i=0;i<count;i++){const pt=randomPointInZone(zone);state.enemies.push({x:pt.x,y:pt.y,hp,glyph:zone.enemy||'😠',zone:zone.id,minR:zone.ring.min,vx:0,vy:0,bounty:2+wave+Math.floor(radTier),hop:Math.random()*0.8,power:enemyPower});} dbg(`[WAVE] ${zone.name} wave ${wave} enemies=${count} hp=${hp.toFixed(1)} power=${enemyPower.toFixed(2)}`); }
...
function update(dt){const p=state.player; let dx=(state.keys.has('d')?1:0)-(state.keys.has('a')?1:0),dy=(state.keys.has('s')?1:0)-(state.keys.has('w')?1:0); const m=Math.hypot(dx,dy)||1;dx/=m;dy/=m;
  const zone=getZone(p.x,p.y), flow=riverPushAt(p.x,p.y); if(zone?.id!==state.currentZone){state.currentZone=zone?.id||null; if(zone)dbg(`[ZONE] entered ${zone.name} wave=${state.waves[zone.id]?.wave||1} spawned=${!!state.waves[zone.id]?.spawned}`);} const horseEq=state.mount==='horse'&&p.unlocked.has('horse'); const horse=metaFor('horse'); const speed=horseEq?2.25*(1+horse.speed*0.1):1,accel=horseEq?4.8*(1+horse.accel*0.1):7.5;
