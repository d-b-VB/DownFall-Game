...
const BUILD_VERSION = 'v0.2.3 build 2026-05-31 00:00 UTC';
...
function resurrect(){ dbg(`[DOWNFALL] run ${state.run} ended; upgrades persist, waves reset`); state.run++; state.player.x=MAP_CENTER.x; state.player.y=MAP_CENTER.y; state.player.vx=0; state.player.vy=0; state.player.hp=14; state.player.weapon='club'; state.player.unlocked=new Set(['club']); state.mount='foot'; state.enemies=[]; state.projectiles=[]; state.ammo={arrows:0,jars:0}; state.diamonds=0; state.pendingReward=null; state.offerNpc=null; state.currentZone=null; for(const z of zones) state.waves[z.id]={wave:1,spawned:false,cleared:false}; }
...
function resolveEnemySpacing(){ const p=state.player; const pr=state.mount==='horse'&&p.unlocked.has('horse')?28:18, er=16; for(let i=0;i<state.enemies.length;i++){ const e=state.enemies[i]; for(let j=i+1;j<state.enemies.length;j++){ const o=state.enemies[j], dx=o.x-e.x, dy=o.y-e.y, d=Math.hypot(dx,dy)||1, min=er*2; if(d<min){ const push=(min-d)*0.5, nx=dx/d, ny=dy/d; e.x-=nx*push; e.y-=ny*push; o.x+=nx*push; o.y+=ny*push; } } const dx=p.x-e.x, dy=p.y-e.y, d=Math.hypot(dx,dy)||1, min=pr+er; if(d<min){ const nx=dx/d, ny=dy/d, push=min-d; e.x-=nx*push*0.65; e.y-=ny*push*0.65; p.x+=nx*push*0.35; p.y+=ny*push*0.35; p.vx+=nx*18; p.vy+=ny*18; p.hp=Math.max(0,p.hp-0.045*(e.power||1)); } } }
...
function resetWorld(){const s={x:MAP_CENTER.x,y:MAP_CENTER.y};state.player={x:s.x,y:s.y,vx:0,vy:0,hp:14,weapon:'club',unlocked:new Set(['club']),swing:0,cannonCooldown:0,lastX:s.x,lastY:s.y}; state.mount='foot'; state.ammo={arrows:0,jars:0}; state.projectiles=[];state.enemies=[];state.terrain=[];state.waves={};state.pendingReward=null;state.offerNpc=null;state.currentZone=null; for(const z of zones){state.waves[z.id]={wave:1,spawned:false,cleared:false};spawnZoneDecor(z);} }
...
function update(dt){const p=state.player; let dx=(state.keys.has('d')?1:0)-(state.keys.has('a')?1:0),dy=(state.keys.has('s')?1:0)-(state.keys.has('w')?1:0); const m=Math.hypot(dx,dy)||1;dx/=m;dy/=m;
  const zone=getZone(p.x,p.y), flow=riverPushAt(p.x,p.y); if(flow.damage)p.hp=Math.max(0,p.hp-flow.damage*dt); if(zone?.id!==state.currentZone){state.currentZone=zone?.id||null; if(zone)dbg(`[ZONE] entered ${zone.name} wave=${state.waves[zone.id]?.wave||1} spawned=${!!state.waves[zone.id]?.spawned}`);} const horseEq=state.mount==='horse'&&p.unlocked.has('horse'); const horse=metaFor('horse'); const speed=horseEq?2.25*(1+horse.speed*0.1):1,accel=horseEq?4.8*(1+horse.accel*0.1):7.5;
...
function drawWaterPath(path,color,highlight,width){ctx.strokeStyle=color;ctx.lineWidth=width;ctx.lineCap='round';ctx.lineJoin='round';ctx.beginPath();path.map(pathPoint).forEach((p,i)=>i?ctx.lineTo(p.x,p.y):ctx.moveTo(p.x,p.y));ctx.stroke();ctx.strokeStyle=highlight;ctx.lineWidth=Math.max(5,width*0.24);ctx.stroke();}
function drawRiver(){drawWaterPath(coldRiverPath,'#77c7ffcc','#d8f2ffbb',46);}
function drawSlingCreek(){drawWaterPath(warmCreekPath,'#69b8d0cc','#c7f0ff99',30);}
