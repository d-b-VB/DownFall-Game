...
function spawnZoneDecor(z){
  ...
  for(let i=0;i<cfg.n;i++){let h,rr; if(cfg.uniform){ const t=(i+0.5)/cfg.n; const start=z.arc[0],end=z.arc[1]; const span=((end-start+12)%12)||12; h=(start + t*span)%12; rr=z.ring.min+(z.ring.max-z.ring.min)*(0.18+0.64*((i*37)%cfg.n)/cfg.n); } else { h=Math.random()*12; if(!inArc(h,z.arc))continue; rr=z.ring.min+(z.ring.max-z.ring.min)*(0.08+0.84*Math.random()); } const th=(h-3)*Math.PI/6; const glyph=cfg.g[(Math.random()*cfg.g.length)|0]; const type=(glyph==='🧱'||glyph==='🪨')?'wall':'tree'; state.terrain.push({x:MAP_CENTER.x+Math.cos(th)*rr,y:MAP_CENTER.y+Math.sin(th)*rr,hp:3,glyph,type,solid:true,stump:false,color:glyph==='♜'?'#000':null});}
}

function resetWorld(){const s={x:MAP_CENTER.x,y:MAP_CENTER.y};state.player={x:s.x,y:s.y,vx:0,vy:0,hp:20,weapon:'club',unlocked:new Set(['club']),swing:0,lastX:s.x,lastY:s.y}; state.mount='foot'; state.projectiles=[];state.enemies=[];state.terrain=[];state.waves={}; for(const z of zones){state.waves[z.id]={wave:1,spawned:false,cleared:false};spawnZoneDecor(z);} for(let i=0;i<28;i++){const a=(i/28)*Math.PI*2;const r=90+Math.random()*180;state.terrain.push({x:MAP_CENTER.x+Math.cos(a)*r,y:MAP_CENTER.y+Math.sin(a)*r,hp:3,glyph:(i%4? '🌳':'🌲'),type:'tree',solid:true,stump:false,color:null});} }
