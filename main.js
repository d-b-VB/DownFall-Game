...
function loop(ts){if(!state.last)state.last=ts;const dt=Math.min(0.033,(ts-state.last)/1000);state.last=ts;if(state.mode==='game'){const p=state.player;p.lastX=p.x;p.lastY=p.y;if(!state.shopOpen&&!state.cheatOpen){state.t+=dt;update(dt);}render();}else if(state.mode==='glyph'){state.t+=dt;renderGlyphTest();}requestAnimationFrame(loop);}
