...
const zones = [
  ...
  { id: 'creek', name: 'Sling Creekside', palette: ['#4f9fc0','#8d9aa2','#333a42'], enemy: '🐸', unlock: 'sling', ring: ring(1,2), arc:[3,12], slowWater:true, deco:'creek' },
...
function spawnWave(zone){ const wave=state.waves[zone.id]?.wave||1,radTier=zone.ring.min/RADIUS_UNIT,difficulty=1+radTier*0.75,enemyPower=1.35*Math.pow(1.1,wave*3+radTier*3); state.waves[zone.id].spawned=true; const count=4+Math.min(12,Math.floor(wave+radTier*1.2)); let hp=5*difficulty*(1+wave*0.08); if(zone.id==='fletcher')hp*=0.55; for(let i=0;i<count;i++){const pt=randomPointInZone(zone);state.enemies.push({x:pt.x,y:pt.y,hp,maxHp:hp,status:{},glyph:zone.enemy||'😠',zone:zone.id,minR:zone.ring.min,vx:0,vy:0,bounty:2+wave+Math.floor(radTier),hop:Math.random()*0.8,power:enemyPower});} dbg(`[WAVE] ${zone.name} wave ${wave} enemies=${count} hp=${hp.toFixed(1)} power=${enemyPower.toFixed(2)}`); }
...
function resolveEnemySpacing(){ const p=state.player; const pr=state.mount==='horse'&&p.unlocked.has('horse')?28:18, er=16; for(let i=0;i<state.enemies.length;i++){ const e=state.enemies[i]; for(let j=i+1;j<state.enemies.length;j++){ const o=state.enemies[j], dx=o.x-e.x, dy=o.y-e.y, d=Math.hypot(dx,dy)||1, min=er*2; if(d<min){ const push=(min-d)*0.5, nx=dx/d, ny=dy/d; e.x-=nx*push; e.y-=ny*push; o.x+=nx*push; o.y+=ny*push; } } const dx=p.x-e.x, dy=p.y-e.y, d=Math.hypot(dx,dy)||1, min=pr+er; if(d<min){ const nx=dx/d, ny=dy/d, push=min-d; e.x-=nx*push*0.65; e.y-=ny*push*0.65; p.x+=nx*push*0.35; p.y+=ny*push*0.35; p.vx+=nx*18; p.vy+=ny*18; p.hp=Math.max(0,p.hp-0.045*(e.power||1)); if(['swamp','axe','marsh'].includes(e.zone))addStatus(p,'poison',0.02*(e.power||1)); } } }
...
function update(dt){const p=state.player; let dx=(state.keys.has('d')?1:0)-(state.keys.has('a')?1:0),dy=(state.keys.has('s')?1:0)-(state.keys.has('w')?1:0); const m=Math.hypot(dx,dy)||1;dx/=m;dy/=m;
  const zone=getZone(p.x,p.y), flow=riverPushAt(p.x,p.y); state.lastWater=flow.type?`${flow.type} d=${flow.d.toFixed(0)} push=${Math.hypot(flow.x,flow.y).toFixed(0)}`:'dry'; if(flow.damage)p.hp=Math.max(0,p.hp-flow.damage*dt); tickStatus(p,dt,true); if(state.pendingReward&&zone?.id!==state.pendingReward.zone){dbg(`[SHOP] left ${state.pendingReward.zone}; dialogue closed`);state.pendingReward=null;state.offerNpc=null;} if(zone?.id!==state.currentZone){state.currentZone=zone?.id||null; if(zone)dbg(`[ZONE] entered ${zone.name} wave=${state.waves[zone.id]?.wave||1} spawned=${!!state.waves[zone.id]?.spawned}`);} const horseEq=state.mount==='horse'&&p.unlocked.has('horse'); const horse=metaFor('horse'); const speed=horseEq?2.25*(1+horse.speed*0.1):1,accel=horseEq?4.8*(1+horse.accel*0.1):7.5;
  ...
  if(state.mouse.down)state.mouse.held+=dt; p.cannonCooldown=Math.max(0,(p.cannonCooldown||0)-dt);
...
function doSwingDamage(){const p=state.player,w=effectiveWeapon(weaponDef());if(w.kind!=='swing')return; const a=Math.atan2(state.mouse.y-innerHeight/2,state.mouse.x-innerWidth/2); const sx=p.x+Math.cos(a)*w.reach,sy=p.y+Math.sin(a)*w.reach;
 ...
 for(const e of state.enemies){if(Math.hypot(e.x-sx,e.y-sy)<45){const spMul=(1+Math.min(2,swingSpeed/220))*w.speedMult; const dmg=(w.pierce*0.5+w.knock*0.25)*spMul*w.damageMult; e.hp-=dmg; const kb=w.knock*spMul*5.2; e.vx=(e.vx||0)+Math.cos(a)*kb; e.vy=(e.vy||0)+Math.sin(a)*kb; dbg(`[SWING:${w.id}] -> enemy ${e.glyph} dmg=${dmg.toFixed(2)} kb=${kb.toFixed(1)} hp=${e.hp.toFixed(2)} ang=${a.toFixed(2)}`);}}
...
function fireCharge(){const w=effectiveWeapon(weaponDef()); const h=Math.min(1,state.mouse.held/1.5),p=state.player; let a=Math.atan2(state.mouse.y-innerHeight/2,state.mouse.x-innerWidth/2),sp=(240+h*460)*w.speedMult;
 ...
 let element=null; if(w.id==='bow'){if(state.ammo.arrows<=0){dbg('[AMMO] buy arrows in Fletcher Village');return;} state.ammo.arrows--; if(state.activeElement&&state.elements[state.activeElement]>0){element=state.activeElement;state.elements[element]--;} glyph='➳';size=22; const wobble=Math.max(0,state.mouse.held-0.7); a+=Math.sin(state.t*20)*wobble*0.06;}
 if(w.id==='ballista'){glyph='➳';size=34;} if(w.id==='cannon'){if((p.cannonCooldown||0)>0){dbg(`[CANNON] cooling ${(p.cannonCooldown||0).toFixed(1)}s`);return;} if(state.mouse.held<0.65){dbg('[CANNON] needs warm-up');return;} glyph='●';size=38;knock=75;pierce=24;sp=520*w.speedMult;p.cannonCooldown=2.0;p.vx-=Math.cos(a)*95;p.vy-=Math.sin(a)*95;} if(w.id==='sling'){if(state.ammo.jars>0&&h>0.7){state.ammo.jars--; if(state.activeElement&&state.elements[state.activeElement]>0){element=state.activeElement;state.elements[element]--;} glyph='🏺';size=22;knock=5.5;pierce=1.2;shards=true;}else{glyph='●';size=12;knock=7.5;pierce=0.5;}}
 const pvx = Math.cos(a)*sp + p.vx; const pvy = Math.sin(a)*sp + p.vy; const pspd=Math.hypot(pvx,pvy);
 state.projectiles.push({x:p.x+Math.cos(a)*24,y:p.y+Math.sin(a)*24,vx:pvx,vy:pvy,life:w.id==='cannon'?4:2.2,glyph,size,knock,pierce,damageMult:w.id==='cannon'?6:(w.damageMult||1),angle:a,speed:pspd,source:w.id,shards,element,pierces:w.id==='cannon'?9:0});
...
function drawStatusIcons(status,x,y){
  const icons=[]; if(status?.fire>0)icons.push(ELEMENT_GLYPHS.fire); if(status?.ice>0)icons.push(ELEMENT_GLYPHS.ice); if(status?.poison>0)icons.push(ELEMENT_GLYPHS.poison);
  ctx.font='12px serif'; icons.forEach((g,i)=>ctx.fillText(g,x+12+i*11,y-14));
}
...
function render(){ctx.clearRect(0,0,innerWidth,innerHeight); drawHexBackground(); drawRiver(); drawSlingCreek();
 ...
 for(const e of state.enemies){const p=projectWorld(e.x,e.y);ctx.font='16px serif';ctx.fillStyle='#fff';ctx.fillText(e.glyph,p.x,p.y);drawStatusIcons(e.status,p.x,p.y);}
 ...
 ctx.save();ctx.translate(pp.x,pp.y); drawHeldWeapon(w,aim); if(state.activeElement&&state.elements[state.activeElement]>0){ctx.font='14px serif';ctx.fillText(ELEMENT_GLYPHS[state.activeElement],Math.cos(aim)*28,Math.sin(aim)*28-10);} ctx.restore(); drawStatusIcons(state.player.status,pp.x,pp.y);
 ...
 const tiers=metaFor(w.id); const showReward=state.pendingReward&&z?.id===state.pendingReward.zone; const specialist=z?.element?`<br>${ZONE_NPCS[z.id]} ${z.id==='swamp'?'Auntie Toxin':z.id==='frost'?'Alchemist':'Wizard'}: <button type="button" data-buy="${z.element}">buy ${z.element} 4♦</button>${z.id==='swamp'?' <button type="button" data-buy="antitoxin">anti-toxin 5♦</button><button type="button" data-buy="poisonResist">resist +10% 10♦</button><button type="button" data-buy="poisonUpgrade">poison +10% 12♦</button>':''}`:''; const elemBtns=`<br>Apply: ${['fire','ice','poison'].map(e=>`<button type="button" data-element="${e}">${e}${state.activeElement===e?'*':''} ${state.elements[e]}</button>`).join(' ')}`; const shopHtml=(showReward?`<br>${state.pendingReward.options.map((o,i)=>`<button type="button" data-reward-index="${i}" ${state.pendingReward.purchased?'disabled':''}>${o.label}</button>`).join(' ')} ${state.pendingReward.zone==='creek'?'<button type="button" data-buy="jars">buy jars 3♦</button>':''}${state.pendingReward.zone==='fletcher'?'<button type="button" data-buy="arrows">buy arrows 1♦</button>':''}${state.pendingReward.purchased?'<button type="button" data-start-wave="1">Start next wave</button>':''}`:'')+specialist+elemBtns; ui.innerHTML=`HP ${state.player.hp.toFixed(1)} · ♦ ${state.diamonds}<br>Zone: ${z?z.name:'Boundary'}<br>R: ${rad} · Clock12: ${clock}<br>Weapon: ${w.id} (${tiers.speed}/${tiers.range}/${tiers.damage}/${tiers.knockback})<br>Ammo: arrows ${state.ammo.arrows} · jars ${state.ammo.jars}<br>Status: poison ${state.player.status.poison||0} · fire ${state.player.status.fire?.toFixed(1)||0}<br>Mount: ${state.mount}<br>Unlocked: ${[...state.player.unlocked].join(', ')}${shopHtml}<br><button type="button" data-reset-world="1">Reset World</button>`;
...
function handleUiAction(e){const mode=e.target.closest('[data-mode]')?.dataset.mode; if(mode){e.preventDefault();e.stopPropagation();setMode(mode);return true;} const glyph=e.target.closest('[data-glyph-weapon]')?.dataset.glyphWeapon; if(glyph){e.preventDefault();e.stopPropagation();state.glyphWeapon=glyph;return true;} const reward=e.target.closest('[data-reward-index]'); if(reward){e.preventDefault();e.stopPropagation();chooseReward(Number(reward.dataset.rewardIndex));return true;} const buy=e.target.closest('[data-buy]'); if(buy){e.preventDefault();e.stopPropagation();buyItem(buy.dataset.buy);return true;} const el=e.target.closest('[data-element]'); if(el){e.preventDefault();e.stopPropagation();state.activeElement=state.activeElement===el.dataset.element?null:el.dataset.element;dbg(`[ELEMENT] applying ${state.activeElement||'none'}`);return true;} if(e.target.closest('[data-start-wave]')){e.preventDefault();e.stopPropagation();startNextWave();return true;} if(e.target.closest('[data-reset-world]')){e.preventDefault();e.stopPropagation();resetWorld();return true;} return false;}
